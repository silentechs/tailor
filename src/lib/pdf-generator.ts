...
export async function generateInvoicePDF(invoice: InvoiceData): Promise<jsPDF> {
  ...
  // Colors (Ghana theme)
  const _primaryColor = '#006B3F'; // Ghana Green
  const _accentColor = '#CE1126'; // Ghana Red
  const _goldColor = '#FCD116'; // Ghana Gold
  const _textColor = '#333333';
  const _lightGray = '#f5f5f5';
