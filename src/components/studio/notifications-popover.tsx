...
import { Bell, Loader2, CheckCircle, Info, AlertTriangle } from 'lucide-react';
import {
    Popover,
    PopoverContent,
    PopoverTrigger,
    ...
    return (
        <Popover>
            <PopoverTrigger asChild>
                <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl hover:bg-white/5 relative">
            ...
            <PopoverContent className="w-80 p-0 bg-zinc-900 border-white/10 text-white" align="end">
                <div className="p-4 border-b border-white/10 flex justify-between items-center">
                    ...
                    {unreadCount > 0 && <Badge className="bg-ghana-gold text-ghana-black text-[10px]">{unreadCount} New</Badge>}
                </div>
                    ...
                    {isLoading ? (
                        ...
                        <div className="divide-y divide-white/5">
                            {notifications?.map((notification: any) => (
                                <div key={notification.id} className={cn("p-4 hover:bg-white/5 transition-colors", !notification.readAt && "bg-white/[0.02]")}>
                                    <div className="flex gap-3">
                                        <div className="mt-1">
                                            {notification.type === 'SUCCESS' ? <CheckCircle className="h-4 w-4 text-emerald-500" /> :
