            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                        ...
                        <CardDescription>Define custom sets of measurements for different garments.</CardDescription>
                    </div>
                    {!isAdding && (
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        />
                                        <Button type="button" variant="outline" onClick={addField}>Add</Button>
                                    </div>
                                </div>
                            </div>
                        ...
                        {templates?.map((template: any) => (
                            ...
                            >
                                <div className="flex items-start justify-between mb-2">
                                    ...
                                    {!template.id && <Badge variant="outline" className="text-[8px] uppercase">Default</Badge>}
                                </div>
                                <div className="flex flex-wrap gap-1 mt-3">
                                    {template.fields.slice(0, 5).map((f: string) => (
                                        <span key={f} className="text-[10px] bg-muted px-1.5 py-0.5 rounded text-muted-foreground uppercase font-mono">
                                            {f.replace(/_/g, ' ')}
                                        </span>
                                    ))}
                                    {template.fields.length > 5 && (
                                        <span className="text-[10px] text-muted-foreground">+{template.fields.length - 5} more</span>
                                    )}
                                </div>
                            </div>
                        ))}
