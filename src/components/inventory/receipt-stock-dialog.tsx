...
export function ReceiptStockDialog({
  open,
  onOpenChange,
  inventoryItems,
}: ReceiptStockDialogProps) {
  const queryClient = useQueryClient();
  const makeRow = () => ({ id: crypto.randomUUID(), itemId: '', quantity: 1, supplier: '' });
  const [rows, setRows] = useState(() => [makeRow()]);

  const mutation = useMutation({
    mutationFn: async (items: any[]) => {
      const res = await fetchApi('/api/inventory/receipt', {
        method: 'POST',
          ...
          {rows.map((row, index) => (
            <div key={row.id} className="grid grid-cols-12 gap-2 items-center">
              <div className="col-span-6">
                <Select value={row.itemId} onValueChange={(val) => updateRow(index, 'itemId', val)}>
                  <SelectTrigger>
