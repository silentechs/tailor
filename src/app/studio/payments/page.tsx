    ...
    return (
        <div className="space-y-12 pb-20">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
                <div>
                    <span className="text-ghana-gold font-black uppercase tracking-[0.3em] text-[10px] mb-4 block">Financial Terminal</span>
                    <h1 className="text-5xl md:text-8xl font-black font-heading tracking-tighter uppercase italic leading-none">
                        Settlement <br /> Hub.
                    </h1>
                </div>
                <div className="flex flex-col items-end">
                    <p className="text-zinc-500 font-bold uppercase tracking-widest text-[10px] mb-2">Account Balance</p>
                    <div className="text-5xl font-black font-heading text-white italic">
                        ₵0.00
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <section className="bg-zinc-900/50 backdrop-blur-xl border border-white/5 rounded-[3rem] p-10">
                        ...
                        <Table>
                            <TableHeader className="border-b-2 border-white/5">
                                <TableRow className="hover:bg-transparent border-none">
                                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-500 py-6">Reference</TableHead>
                                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Service</TableHead>
                                    <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Amount</TableHead>
                            ...
                            <TableBody>
                                {invoices.map((inv: any) => (
                                    <TableRow key={inv.id} className="border-b border-white/5 hover:bg-white/5 group transition-colors">
                                        ...
                                        <TableCell>
                                            <Badge className={cn(
                                                ...
                                                inv.status === 'PAID' ? "bg-emerald-500/20 text-emerald-500" : "bg-ghana-gold/20 text-ghana-gold"
                                            )}>
                                                {inv.status}
                                            </Badge>
                                        </TableCell>
                                        <TableCell className="text-right">
                                            <Button variant="ghost" size="icon" className="h-12 w-12 rounded-2xl hover:bg-white/10 group-hover:bg-white/10">
                                            ...
                                            {inv.status !== 'PAID' && (
                                                ...
                                                >
                                                    {processing === inv.id ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Settle'}
                                                </Button>
                                            )}
                                        </TableCell>
                                    </TableRow>
                        ...
                        {invoices.length === 0 && (
                            <div className="py-20 text-center">
                                <p className="text-zinc-600 font-bold uppercase tracking-widest text-xs italic">No invoice history found</p>
                            </div>
                        )}
                    </section>
                </div>
                ...
                <div className="lg:col-span-4 space-y-8">
                        ...
                        <div className="space-y-8">
                            {payments.map((p: any) => (
                                <div key={p.id} className="flex items-center justify-between group">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-xs font-black uppercase text-white mb-0.5">{p.paymentMethod}</p>
                                            <p className="text-[10px] font-bold text-zinc-600 uppercase italic">
                                                {new Date(p.paidAt).toLocaleDateString('en-GH', { day: 'numeric', month: 'short' })}
                                            </p>
                                        </div>
                                    </div>
                            ...
                            {payments.length === 0 && (
                                <div className="py-10 text-center">
                                    <p className="text-zinc-600 font-bold uppercase tracking-widest text-[10px]">No recent transactions</p>
                                </div>
                            )}
                        </div>

                        <Button variant="ghost" className="w-full mt-12 rounded-2xl h-14 border border-white/5 font-black uppercase tracking-widest text-[10px] hover:bg-white/5">
                            View Full Ledger
                        </Button>
                    </section>
                    ...
                    <div className="p-8 bg-zinc-900/50 border border-white/5 rounded-[2.5rem] relative overflow-hidden group">
                        ...
                        <h4 className="text-xs font-black uppercase tracking-widest text-zinc-400 mb-4">Secure Gateway</h4>
                        <p className="text-zinc-500 font-bold leading-relaxed italic text-xs mb-6">
                            All transactions are end-to-end encrypted via Paystack securely. We do not store your card details.
                        </p>
