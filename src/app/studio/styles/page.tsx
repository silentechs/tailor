    ...
    return (
        <div className="space-y-12 pb-20">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
                <div>
                    <span className="text-ghana-gold font-black uppercase tracking-[0.3em] text-[10px] mb-4 block">Visual Library</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {wishlist.map((item: any, idx: number) => (
                    ...
                    >
                        ...
                        <div className="aspect-[4/5] overflow-hidden relative">
                            <img
                                src={item.portfolioItem.images[0] || 'https://images.unsplash.com/photo-1594938298603-c8148c4dae35?q=80&w=800'}
                        ...
                        <div className="p-8">
                            ...
                            <div className="flex flex-wrap gap-2 mb-8">
                                {item.portfolioItem.tags?.slice(0, 3).map((tag: string) => (
                                    <Badge key={tag} variant="outline" className="text-[9px] uppercase font-black tracking-tighter border-white/10 rounded-full px-3 py-0.5">
                                        {tag}
                                    </Badge>
                                ))}
                            </div>
                            ...
                            <div className="flex items-center gap-4">
                                <Button asChild className="flex-1 bg-white/5 hover:bg-white/10 border-white/5 h-14 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all">
                                    <Link href={`/showcase/${item.portfolioItem.tailor?.showcaseUsername || '#'}`}>
                                        View Tailor
                                    </Link>
                                </Button>
                                <Button size="icon" className="h-14 w-14 rounded-2xl bg-white/5 hover:bg-white/10 border-white/5 text-zinc-400">
                ...
                {wishlist.length === 0 && (
                    <div className="col-span-full py-40 border-2 border-dashed border-white/5 rounded-[3rem] flex flex-col items-center justify-center text-center">
                        ...
                        <h3 className="text-xl font-black font-heading uppercase mb-2">Your library is empty</h3>
                        <p className="text-zinc-600 font-bold uppercase tracking-widest text-xs">Explore the gallery to save inspirations.</p>
                        <Button className="mt-8 rounded-full px-8 bg-ghana-gold text-ghana-black hover:bg-ghana-gold/90 font-black" asChild>
