    ...
    return (
        <div className="space-y-12">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-5xl font-black font-heading uppercase tracking-tighter italic">Garment Tracker</h1>
                        ...
                        {selectedOrder ? (
                            ...
                            >
                                ...
                                <div className="flex flex-col md:flex-row justify-between gap-8 mb-16 pb-12 border-b border-white/5">
                                    <div>
                                        <h2 className="text-4xl font-black font-heading uppercase mb-2 tracking-tighter">Garment Stage</h2>
                                        <div className="flex flex-wrap gap-4 mt-4">
                                            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-xl">
                                                <MapPin className="h-4 w-4 text-ghana-gold" />
                                                <span className="text-xs font-bold uppercase text-zinc-400">{orders.find((o: any) => o.id === selectedOrder).organization.city || 'Accra, GH'}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[10px] font-black uppercase text-zinc-600 tracking-widest mb-1">Current Status</p>
                                ...
                                <div className="relative space-y-12">
                                    ...
                                    {orders.find((o: any) => o.id === selectedOrder).timeline.map((step: any, idx: number) => (
                                ...
                                <div className="mt-20 pt-10 border-t border-white/5 space-y-12">
                                    {orders.find((o: any) => o.id === selectedOrder).status === 'DELIVERED' && (
                                        <div className="bg-ghana-gold/5 rounded-[2rem] p-8 border border-ghana-gold/10">
                                            <h4 className="text-xl font-black font-heading uppercase mb-4 text-ghana-gold">Share your experience</h4>
                                            <p className="text-zinc-500 text-sm font-bold mb-6">How was the fit and finish of your {orders.find((o: any) => o.id === selectedOrder).garmentType}?</p>
                                                ...
                                                {[1, 2, 3, 4, 5].map((star) => (
                                                    <button
                                                        ...
                                                        className={cn(
                                                            ...
                                                            rating >= star ? "bg-ghana-gold text-ghana-black" : "bg-white/5 text-zinc-600 hover:bg-white/10"
                                                        )}
                                                    >
                                    ...
                                    {/* Payment Section */}
                                    {Number(orders.find((o: any) => o.id === selectedOrder).totalAmount) > Number(orders.find((o: any) => o.id === selectedOrder).paidAmount) && (
                                        <div className="bg-zinc-950/50 rounded-[2rem] p-8 border border-white/5 flex flex-col items-center text-center">
                                            <Badge variant="outline" className="mb-4 border-ghana-gold text-ghana-gold">Payment Pending</Badge>
                                            <p className="text-3xl font-black font-heading mb-2">
                                                GH₵ {(Number(orders.find((o: any) => o.id === selectedOrder).totalAmount) - Number(orders.find((o: any) => o.id === selectedOrder).paidAmount)).toFixed(2)}
                                            </p>
                                            <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-8">Remaining balance for this order</p>
                                            <Button
                                                ...
                                                onClick={() => payNowMutation.mutate(orders.find((o: any) => o.id === selectedOrder))}
                                            >
                                    ...
                                    <div className="flex justify-center">
                                        <Button variant="outline" className="rounded-2xl h-14 border-white/10 hover:bg-white/5 font-black uppercase tracking-widest text-[10px] px-10 gap-3">
                            ...
                            <div className="flex flex-col items-center justify-center h-full min-h-[500px] border-2 border-dashed border-white/5 rounded-[3rem] text-zinc-700">
                                ...
                                <p className="font-bold uppercase tracking-[0.2em] text-sm">Select an order to track progress</p>
                            </div>
                        )}
                    </AnimatePresence>
                </div>
