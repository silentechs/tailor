...
export async function POST(request: Request) {
    try {
        ...
        return NextResponse.json({ success: false, error: 'Failed to create voice log' }, { status: 500 });
    }
}
...
export async function GET() {
    try {
        ...
        return NextResponse.json({ success: false, error: 'Failed to fetch voice logs' }, { status: 500 });
    }
}
