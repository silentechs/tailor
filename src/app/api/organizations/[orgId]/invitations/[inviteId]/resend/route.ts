...
export async function POST(
    ...
    try {
        ...
        if (invitation.status !== 'PENDING' && invitation.status !== 'EXPIRED') {
            return NextResponse.json({ success: false, error: 'Only pending or expired invitations can be resent' }, { status: 400 });
        }

        // Rate limiting: Check if resent in the last 60 seconds
        const oneMinuteAgo = new Date(Date.now() - 60000);
        if (invitation.updatedAt && invitation.updatedAt > oneMinuteAgo) {
            return NextResponse.json({ success: false, error: 'Please wait a minute before resending again' }, { status: 429 });
        }
