  ...
  try {
    ...
    if (user.role !== 'CLIENT') {
      return NextResponse.json(
        { success: false, error: 'Only client accounts can claim client records' },
        { status: 403 }
      );
    }
    ...
    if (tracking.client.user) {
      return NextResponse.json(
        { success: false, error: 'This client record is already linked' },
        { status: 400 }
      );
    }
    ...
    if (user.linkedClientId) {
      return NextResponse.json(
        { success: false, error: 'Your account is already linked to a client record' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to claim client record' },
      { status: 500 }
    );
  }
}
