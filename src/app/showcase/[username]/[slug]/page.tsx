    ...
    if (error || !project) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
                ...
                <Button variant="link" onClick={() => router.back()} className="mt-4 text-primary font-bold">
                    Return to Showcase
                </Button>
            </div>
        );
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <header className="fixed top-0 left-0 w-full h-20 bg-white/10 backdrop-blur-md z-50 px-8 flex items-center justify-between border-b border-white/10">
                ...
                <div className="flex gap-4">
                    <Button variant="ghost" size="icon" className="rounded-full bg-white/20 text-white hover:bg-white/30" onClick={() => toast.success('Saved to inspirations')}>
                        ...
                        <div className="flex items-center gap-6">
                            <div className="flex items-center gap-3">
                                <div className="h-10 w-10 rounded-full overflow-hidden border-2 border-white/20">
                                    <Image src={project.tailor.profileImage || `https://api.dicebear.com/7.x/avataaars/svg?seed=${project.tailor.name}`} alt={project.tailor.name} width={40} height={40} className="object-cover" />
                                </div>
                                <span className="text-white font-bold text-lg">{project.tailor.businessName || project.tailor.name}</span>
                            </div>
                    ...
                    <div className="lg:col-span-2 space-y-16">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {project.images.map((img: string, i: number) => (
                                ...
                                >
                                    <Image src={img} alt={`${project.title} detail ${i + 1}`} fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                    ...
                    <div className="space-y-12">
                        <Card className="rounded-[2.5rem] p-10 bg-white border-none shadow-xl space-y-8 sticky top-32">
                            <div className="space-y-4">
                                <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Attributes</h4>
                                <div className="flex flex-wrap gap-2">
                                    <Badge variant="secondary" className="bg-slate-100 text-slate-600 border-none font-bold text-[10px] uppercase tracking-widest px-4 py-1.5 rounded-full">
                            ...
                            <div className="space-y-6">
                                <p className="text-slate-500 font-medium text-sm leading-relaxed">
                                    Inspired by this piece? Contact {project.tailor.businessName || project.tailor.name} to discuss your custom vision.
                                </p>
                                <Button
            ...
            <footer className="py-20 bg-slate-100 border-t border-slate-200">
                <div className="max-w-7xl mx-auto px-8 text-center space-y-4">
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Made with Heart In Ghana</p>
                    <p className="text-slate-500 font-medium">© {new Date().getFullYear()} StitchCraft Global Artisan Suite.</p>
                </div>
            </footer>
