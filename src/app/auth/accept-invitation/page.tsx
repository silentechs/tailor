  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) {
        const err = await res.json();
        if (res.status === 401) {
          // Check if user already exists from inviteData
          const email = inviteData?.data?.email;
          const userExists = inviteData?.data?.userExists;

          if (userExists) {
            // Redirect to Login if account exists
            const loginTarget = email
              ? `/auth/login?email=${encodeURIComponent(email)}&callbackUrl=${encodeURIComponent(window.location.href)}`
              : `/auth/login?callbackUrl=${encodeURIComponent(window.location.href)}`;
            router.push(loginTarget);
          } else {
            // Redirect to Register if new user
            const registerTarget = email
      ...
      <div className="flex items-center justify-center min-h-screen p-4 bg-muted/30">
        <Card className="w-full max-w-md shadow-2xl border-none rounded-3xl overflow-hidden">
          <CardHeader className="bg-destructive/10 text-center py-8">
            ...
            <CardTitle className="text-2xl font-black font-heading text-destructive">
              Invitation Invalid
            </CardTitle>
      ...
      <div className="flex items-center justify-center min-h-screen p-4 bg-muted/30">
        <Card className="w-full max-w-md shadow-2xl border-none rounded-3xl overflow-hidden animate-in fade-in zoom-in duration-500">
          <CardHeader className="bg-primary/10 text-center py-10">
            ...
            <CardTitle className="text-3xl font-black font-heading text-primary">
              Success!
            </CardTitle>
    ...
    <div className="flex items-center justify-center min-h-screen p-4 bg-gradient-to-br from-primary/5 via-background to-secondary/5">
      <Card className="w-full max-w-lg shadow-2xl border-none rounded-[2.5rem] overflow-hidden bg-background/80 backdrop-blur-xl">
        <CardHeader className="text-center pt-12 pb-6 px-10">
          ...
          <CardTitle className="text-4xl font-black font-heading tracking-tight mb-2">
            You're Invited!
          </CardTitle>
          <CardDescription className="text-xl font-medium">
            Join{' '}
            <span className="text-foreground font-black underline decoration-primary/30 decoration-4 underline-offset-4">
